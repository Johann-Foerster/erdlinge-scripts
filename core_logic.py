"""
Core logic extracted from CLI scripts for reuse in Gradio app
"""
...
import numpy as np
import glob, os, re, csv, tempfile
from typing import List, Dict, Any, Tuple


def get_pages(filename):
    """Common function to extract pages from PDF files using Tika"""
...
def parse_float(float_str_eu: str):
    """Common function to parse German-formatted floats"""
...
def format_float(number):
    """Format float for German locale (comma as decimal separator)"""
    integer_part, decimal_part = f"{number:.2f}".split(".")
    return f"{integer_part},{decimal_part}"


def find_index(data, element):
    """Helper function to find index of element in data"""
...
# AAG Erstattungen Logic
def process_aag_erstattungen(pdf_files: List[str], year: str = "2024") -> str:
    """
    Process AAG reimbursement PDFs and return CSV content as string
    """
    ROW_SUM = "Summe"
    erstattungen_u1 = {}
    erstattungen_u2 = {}
    ...
    csv_content = []
    csv_content.append("U1")
    csv_content.append(";".join(["Name"] + titles + [ROW_SUM]))
...
@dataclass
class Page:
    """Class to represent a payroll page"""
    ...
    def __init__(self, page: str):
        ...
        self.gruppe_stufe = (
            self.gruppe_stufe_calc(self.line_with(TVOD)) if TVOD in page else "-"
        )
    ...
    def gruppe_stufe_calc(self, line: str):
...
def process_abrechnungen(pdf_files: List[str], year: str = "2024") -> str:
    """
    Process payroll PDFs and return Excel file path
    """
    pages = []
    for pdf_file in pdf_files:
            ...
            for table in tables:
                ...
                if table["name"] in data[page.month][page.name]:
                    ...
                    if old_datapoint != datapoint:
                        print(
                            f"Changed {table['name']} for {page.name}, month {page.month}, old={old_datapoint}, new={datapoint}, RR={page.is_rueckrechnung}"
                        )
    ...
    with ExcelWriter(temp_file.name, engine="openpyxl", mode="w") as writer:
        for table in tables:
            df = DataFrame()
            for month in months:
                for employee in names:
                    df.loc[employee, month] = (
                        ...
                        if employee in data[month] and table["name"] in data[month][employee]
                        else 0
                    )
...
# AG Belastung and Lohnjournal functions will be similar...
...
def process_ag_belastung(pdf_files: List[str], year: str = "2024") -> str:
    """
    Process AG burden PDFs and return Excel file path
    """
    if len(pdf_files) != 1:
        raise ValueError("Expected exactly one PDF file for AG Belastung")
...
def process_lohnjournal(pdf_files: List[str], year: str = "2024") -> str:
    """
    Process payroll journal PDFs and return Excel file path
    """
    if len(pdf_files) != 1:
        raise ValueError("Expected exactly one PDF file for Lohnjournal")
